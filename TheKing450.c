/*
*============================================================================
*TheKing450 - WinBoard/XBoard Protocol Only
...
*Protocol: WinBoard/XBoard v2 ONLY (UCI removed to avoid dual-protocol bugs)
...
*TheKing450.c-o TheKing450-lpthread-lm
*
*Windows(ucrt64):
*gcc -O3 -march=native -DNDEBUG -DIS_64BIT -DUSE_POPCNT \
*TheKing450.c -o TheKing450.exe -lm -lpthread
*
*FEATURES:
*---------
...
*-Benchmark command
*-UCI protocol (removed to prevent dual-protocol conflicts)
*
*BUGFIXES APPLIED:
*-----------------
*-Fixed stack buffer overflow (line 7102: increased from 215 to MAX_PLY+110)
*-Fixed pos.st not resetting between games (added reset in "new" command)
*-Fixed pos.st not resetting in setboard command
*-Removed UCI protocol to eliminate dual-protocol "two captain" bugs
*
*============================================================================
*/
#define NDEBUG
...
pos.stackAllocation=malloc(63+(MAX_PLY+110)*sizeof(Stack));
pos.stack=(Stack*)(((uintptr_t)pos.stackAllocation+0x3f)&~0x3f);
...
strcpy(fen,StartFEN);
pos.st=pos.stack+100;  /* Reset stack pointer for new game */
pos_set(&pos,fen,0);
pos.rootKeyFlip=pos.st->key;
wb_force_mode=0;
...
pos.st=pos.stack+100;  /* Reset stack pointer for new position */
pos_set(&pos,str,0);
pos.rootKeyFlip=pos.st->key;
}
...
/* TheKing450 is WinBoard/XBoard only - UCI not supported */
else if(strcmp(token,"uci")==0){
printf("Error (UCI not supported): TheKing450 is WinBoard/XBoard only\n");
printf("Error: Please use WinBoard protocol (feature command sent at startup)\n");
fflush(stdout);
}
